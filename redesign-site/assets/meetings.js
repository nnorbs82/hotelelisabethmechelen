(() => {
  ...
  function storyTabs(entries){
    const l=t();
    return `<div class="meeting-story-tabs" role="tablist" aria-label="${escape(l.choose)}">${entries.map(([id,item],index)=>{
  ...
  function showPhoto(index){
    ...
    const list=photos(activeMeeting);const count=Math.max(list.length,1);const next=(index+count)%count;galleryIndex.set(activeMeeting,next);
  ...
  document.addEventListener('click',event=>{
    ...
    if(tab){activeMeeting=tab.dataset.meetingTab;render(activeMeeting);return;}
